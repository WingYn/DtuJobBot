...
def feature_selector_lr(X,y,cvf=10,features_record=None,loss_record=None):
    ...
    for feature in range(0,X.shape[1]):
        if np.where(selected_features==feature)[0].size==0:
            trial_selected = np.concatenate((selected_features,np.array([feature])),1).astype(int)
    ...
    if best_feature!=False:
        ...
        loss_record = np.concatenate((loss_record,np.array([min_loss])),1)
...
def dbplot(classifier, X, y, grid_range, resolution=100):
    ...
    hold(True)
    for c in range(C):
        cmask = (y==c).A.ravel(); plot(X[cmask,0], X[cmask,1], '.', color=C_colors[c], markersize=10)
...
def rocplot(p, y):
    ...
    FPR = 1 - TNR
    onemat = np.mat([1])
    TPR = np.bmat('onemat; TPR'); FPR = np.bmat('onemat; FPR')
...
def gauss_2d(centroid, ccov, std=1, points=100):
